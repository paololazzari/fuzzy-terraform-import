...
func TestEC2SubnetProperties(t *testing.T) {
	region := "us-east-1"
	profile := "local"
	...
	obj := fuzzyec2.EC2SubnetProperties(svc)
	assert := assert.New(t)
	...
	assert.NotEmpty(obj[0]["VpcId"], "VpcId should not be empty")
	assert.NotEmpty(obj[0]["CidrBlock"], "CidrBlock should not be empty")
	assert.NotEmpty(obj[0]["AvailabilityZone"], "AvailabilityZone should not be empty")
}
