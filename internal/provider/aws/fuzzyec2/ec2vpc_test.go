...
func TestEC2VPCProperties(t *testing.T) {
	region := "us-east-1"
	profile := "local"
	...
	obj := fuzzyec2.EC2VPCProperties(svc)
	assert := assert.New(t)
	...
	assert.NotEmpty(obj[0]["State"], "State should not be empty")
	assert.NotEmpty(obj[0]["IsDefault"], "IsDefault should not be empty")
}
